...
def check_cookies_batch(cookies):
    ...
    # Optimized concurrent checking with higher workers for better performance
    max_workers = min(10, len(cookies))  # Increased to 10 concurrent requests for better performance
    print(f"Using {max_workers} concurrent workers")

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all cookie checks
        future_to_cookie = {executor.submit(check_single_cookie_optimized, cookie, i): (cookie, i)
